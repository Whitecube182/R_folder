...
Join data set 1 & 2 and check correlation on each series. This will involve calculating the yearly return from the indicies, and join them. And check correlation and covariance.
...
The Graphs above shows the development for the stocks and the benchmark (normalized to 100 for comparison).
...
Another way that makes it a bit easier to understand the development for each country would be:
...
For good messure we can also present the GDP and the population in this way:
...
Before we continue to the conclusion we need to look at the correlation between this data. In the current state we can already see that all of the data are in upwards trends. Population are all increasing and so are returns for the countries indices and also the benchmark (FTSE World). We can also conclude from the graphs that indices are fluctuating a lot more compared to population and GDP.
