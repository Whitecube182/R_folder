...
Above we are simply cleaning out and making usable data frames for later in the project. There are simply two frames we are currently interested in here. Is population_clean and gdp_clean. We removed all years before 2003 as we are using this data together with the data from dataset one.

## Join dataset one and two! ----------THIS IS WHERE I AM

Now we have a decent amount of dataframes from the two first datasets we want to join them together.
